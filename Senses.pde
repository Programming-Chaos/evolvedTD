...
class Sensory_Systems {
  ...
  Sensory_Systems(Genome g) {
    ...
    Set_Pain(pain, 0, abs((float)Utilities.Sigmoid(g.sum(painDampeningTrait), 5,
                          1)), abs((float)Utilities.Sigmoid(g.sum(painThresholdTrait), 5,
                                   100))); /*Has pain, pain amount currentlyu, pain_dampening, pain_threshold*/

    num_feelers = (int)abs((float)Utilities.Sigmoid(g.sum(expressedFeelers), 1,
                           38));
  ...
  void Set_Feeler(Genome g) {
    for (int i = 0; i < num_feelers; i++) {
      ...
      apend_angles[i] = abs((float)Utilities.Sigmoid(g.sum(feelers.get(i).angle), 3,
                            2 * PI));
      apend_length[i] = abs((float)Utilities.Sigmoid(g.sum(feelers.get(i).length), 5,
                            100));
  ...
  void Update_Sense(float x, float y, float angle, float evolved_angle,
                    float evolved_length, int i) {
  ...
  void Draw_Feeler(float x, float y, float angle, float evolved_angle,
                   float evolved_length) {
  ...
  void Set_Pain(boolean _pain, float _curr_pain, float _pain_dampening,
                float _pain_threshold) {
    pain = _pain;
  ...
  void Set_Stats(creature c) {
    ...
    if (energy) {
      brain_array[b_energy_reproduction] = c.energy_reproduction /
                                           c.max_energy_reproduction;
